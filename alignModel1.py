...
#for (f_i, e_j) in fe_count.keys():
#  t[(f_i,e_j)] = fe_count[(f_i, e_j)]/float(e_count[e_j])
...
for iteration in range(num_iteration):
  ...
  for (n, (f, e)) in enumerate(bitext):   #for all sentence pairs (e, f) do
    for f_i in set(f):                     #for all words f_i in f do
      ...
      for e_j in set(e):                        #for each <f,e> in count do
        #for f_i in set(f):
        count[(f_i, e_j)] += t[(f_i, e_j)]/s_total                   #count[<f(i)^(n), e(j)^(n)>]++ 
        total[e_j] += t[(f_i, e_j)]/s_total                          #count[e(j)^n]++
    if n % 500 == 0:
      sys.stderr.write(".")
  ...
  for (f, e) in count.keys():
    t[(f,e)] = (count[(f, e)])/(total[e])  ##p(f|e) = count(<f, e>)/count(e)
...
#print ''.join(set(orys))
tuPar = zip(set(orys), set(kazakh), set(measu))
#print (tuPar)
for (n, (f, e)) in enumerate(bitext):
